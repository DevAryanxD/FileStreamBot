...
from FileStream.utils.bot_utils import is_user_banned, is_user_exist, is_user_joined, gen_linkx, is_channel_banned, is_channel_exist, is_user_authorized
...
async def private_receive_handler(bot: Client, message: Message):
    ...
    try:
        ...
        reply_markup, stream_text = await gen_linkx(_id=inserted_id, name=[FileStream.username, FileStream.fname])
...
async def channel_receive_handler(bot: Client, message: Message):
    ...
    try:
        ...
        # Extract links from stream_text
        file_info = await db.get_file(inserted_id)
        mime_type = file_info['mime_type']
        page_link = f"{Server.URL}watch/{inserted_id}"
        stream_link = f"{Server.URL}dl/{inserted_id}"
        
        # Set caption in the required format
        caption_text = f"Stream: {page_link}\nDDL: {stream_link}" if "video" in mime_type else f"DDL: {stream_link}"
